...
export class Game implements GameInterface {
  ...
  constructor(id: String, playerO: PlayerInterface, playerX: PlayerInterface, board: BoardInterface, status: GameStatusType) {
    this._id = id
  ...
  public movePlayerById(idPlayer: String, x: number, y: number): MovementResultType {
    if (this.playerX.id === idPlayer) {
      return this.movePlayerX(x, y)
    } else if (this.playerO.id === idPlayer) {
      return this.movePlayerO(x, y)
    } else {
      return MovementResultType.PlayerNotAssignedToGame
    }
  }
